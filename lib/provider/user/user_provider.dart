...
import 'package:dni_ecommerce_v2/ui/dialog/error_dialog.dart';
import 'package:dni_ecommerce_v2/ui/dialog/warning_dialog_view.dart';
import 'package:dni_ecommerce_v2/utils/app_progress_dialog.dart';
...
import 'package:dni_ecommerce_v2/viewobject/holder/user_login_parameter_holder.dart';
import 'package:dni_ecommerce_v2/viewobject/holder/user_register_parameter_holder.dart';
...
class UserProvider extends AppProvider {
  ...
  late User holderUser;
  bool isCheckBoxSelect = true;
  // late ShippingCountry selectedCountry;
  // late ShippingCity selectedCity;

  AppResource<User> _user = AppResource<User>(AppStatus.NOACTION, '', User());
  AppResource<User> _holderUser =
      AppResource<User>(AppStatus.NOACTION, '', User());
  AppResource<User> get user => _user;

  AppResource<ApiStatus> _apiStatus =
      AppResource<ApiStatus>(AppStatus.NOACTION, '', ApiStatus());
  AppResource<ApiStatus> get apiStatus => _apiStatus;

  late StreamSubscription<AppResource<User>> subscription;
  late StreamController<AppResource<User>> userListStream;
  ...
  Future<void> handleFirebaseAuthError(BuildContext context, String email,
    ...
    // final fb_auth.UserCredential providers =
    //     await _firebaseAuth.signInWithEmailAndPassword(email:email,password: '');

    // final String provider = providers.user.toString();
    ...
    // else if (provider.contains(AppConst.googleAuthProvider)) {
    //   showDialog<dynamic>(
    //       context: context,
    //       builder: (BuildContext context) {
    //         return WarningDialog(
    ...
    // else if (provider.contains(AppConst.facebookAuthProvider)) {
    //   showDialog<dynamic>(
    //       context: context,
    //       builder: (BuildContext context) {
    //         return WarningDialog(
    ...
    // else if (provider.contains(AppConst.appleAuthProvider)) {
    //   showDialog<dynamic>(
    //       context: context,
    //       builder: (BuildContext context) {
    //         return WarningDialog(
  ...
  // Future<AppResource<User>> _submitLoginWithAppleId(fb_auth.User user) async {
  //   if (user != null) {
  ...
  // Future<AppResource<User>> _submitLoginWithGoogleId(fb_auth.User user) async {
  //   if (user != null) {
  //     String email = user.email;
  //     if (email == null || email == '') {
  //       // Apple and Facebook
  ...
  //   // Get Keys
  //   const String yourClientId = AppConfig.fbKey;
  ...
  // Future<AppResource<User>> _submitLoginWithFacebookId(
